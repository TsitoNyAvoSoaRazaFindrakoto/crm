...
public class DatabaseCustomUtil {
    ...
    public List<CustomerLoginInfo> generateCustomers(int count) {
        User admin = userService.findFirst();
        List<CustomerLoginInfo> customersInfos = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            String username = faker.name().name();
            String email = username.replace(" ", "") + "@gmail.com";
    ...
    public List<Budget> buildBudget(List<BudgetDtoCsv> budgetDtoCsvs, HashMap<String, Integer> mapCustomer, StringBuilder errorMessage) {
        ...
        for (int i = 0; i < budgetDtoCsvs.size(); i++) {
            ...
            if (customerId == null) {
                errors.add(String.format("Row %d: Customer email '%s' not found for budget.", i + 2, budgetDtoCsv.getCustomerEmail()));
            } else {
                Customer customer = new Customer();
    ...
    public List<Ticket> buildTicket(List<TicketLeadDtoCsv> ticketLeadDtoCsvs, HashMap<String, Integer> mapCustomer, StringBuilder errorMessage) {
        User admin = userService.findFirst();
        ...
        for (int i = 0; i < ticketLeadDtoCsvs.size(); i++) {
            ...
            if (ticketLeadDtoCsv.getType().equalsIgnoreCase("ticket") || ticketLeadDtoCsv.getType().equalsIgnoreCase("tickets")) {
                Ticket ticket = new Ticket();
                Integer customerId = mapCustomer.get(ticketLeadDtoCsv.getCustomerEmail());

                if (customerId == null) {
                    errors.add(String.format("Row %d: Customer email '%s' not found for ticketLeadCsv.", i + 2, ticketLeadDtoCsv.getCustomerEmail()));
                } else {
    ...
    public List<Lead> buildLead(List<TicketLeadDtoCsv> ticketLeadDtoCsvs, HashMap<String, Integer> mapCustomer, StringBuilder errorMessage) {
        User admin = userService.findFirst();
        ...
        for (int i = 0; i < ticketLeadDtoCsvs.size(); i++) {
            TicketLeadDtoCsv ticketLeadDtoCsv = ticketLeadDtoCsvs.get(i);
            if (ticketLeadDtoCsv.getType().equalsIgnoreCase("lead") || ticketLeadDtoCsv.getType().equalsIgnoreCase("leads")) {
                Lead lead = new Lead();
                Integer customerId = mapCustomer.get(ticketLeadDtoCsv.getCustomerEmail());
    ...
    public void importCsvAndSave(List<BudgetDtoCsv> budgetDtoCsvs, List<TicketLeadDtoCsv> ticketLeadDtoCsvs, List<CustomerDtoCsv> customerDtoCsvs) throws SQLDataException {
